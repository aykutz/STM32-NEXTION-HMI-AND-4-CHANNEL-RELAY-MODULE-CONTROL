...
/* USER CODE BEGIN PV */
char rxBuffer[50];
int counter=0;  // I used for debugging process, not neceserry
/* USER CODE END PV */
...
void ReadData()
{
	HAL_UART_Receive(&huart2, (uint8_t*)rxBuffer,50,100); // take data from to nextion display


    if(rxBuffer[0]==0x65 && rxBuffer[1]==0x00 && rxBuffer[2]==0x02 && rxBuffer[3]==0x01 && rxBuffer[4]==0xFF
    	&& rxBuffer[5]==0xFF && rxBuffer[6]==0xFF)
    {
    	// 65 00 02 02 01 ff ff ff -> button 1 id, like if push button
    	counter++;
    	...
    	&& rxBuffer[5]==0xFF && rxBuffer[6]==0xFF)
    {
    	...
    	// 65 00 02 03 01 ff ff ff -> button 2 id
    ...
    if(rxBuffer[0]==0x65 && rxBuffer[1]==0x00 && rxBuffer[2]==0x04 && rxBuffer[3]==0x01 && rxBuffer[4]==0xFF
    	&& rxBuffer[5]==0xFF && rxBuffer[6]==0xFF)
    {
    	// 65 00 02 04 01 ff ff ff -> button 3 id
    ...
    if(rxBuffer[0]==0x65 && rxBuffer[1]==0x00 && rxBuffer[2]==0x05 && rxBuffer[3]==0x01 && rxBuffer[4]==0xFF
    	&& rxBuffer[5]==0xFF && rxBuffer[6]==0xFF)
    {
    	// 65 00 02 04 01 ff ff ff -> button 4 id
    ...
    for(int i=0; i<50; i++) // you should clear rxBuffer every push to button
    	rxBuffer[i]='\0';
...
int main(void)
{
  ...
  MX_USART2_UART_Init();
  /* USER CODE BEGIN 2 */
  HAL_GPIO_WritePin(GPIOD, relay1_Pin | relay2_Pin | relay3_Pin | relay4_Pin, GPIO_PIN_RESET); // reset all pin
  /* USER CODE END 2 */
